...
extension TopTenShortStoryCVC {
    struct Layout {
        // Ranking number dimensions
        static let rankingNumberWidth: CGFloat = 60
        static let rankingNumberLeftOffset: CGFloat = -15 // Negative to overlap with cover

        // Standard dimensions from ShortStoryCVC
...
class TopTenShortStoryCVC: UICollectionViewCell {
    ...
    private let rankingNumberLabel = UILabel() // Netflix-style ranking number
    ...
    private func setupUI() {
        ...
        setupContainerView()
        setupRankingNumberLabel() // Setup number BEFORE cover so it appears behind
        setupCoverImageView()
    ...
    private func setupRankingNumberLabel() {
        // Netflix-style large number
        rankingNumberLabel.font = UIFont.systemFont(ofSize: 92, weight: .heavy)
        rankingNumberLabel.textColor = UIColor.secondaryLabel.withAlphaComponent(0.24)
        rankingNumberLabel.textAlignment = .center
        rankingNumberLabel.isAccessibilityElement = false

        // Prefer no shadow for a watermark/background treatment
        rankingNumberLabel.layer.shadowOpacity = 0

        containerView.addSubviewForConstraints(rankingNumberLabel)
    }
    ...
    private func setupConstraints() {
        NSLayoutConstraint.activate([
            ...
            // Ranking number - positioned to the left, overlapping with cover
            rankingNumberLabel.leadingAnchor.constraint(equalTo: containerView.leadingAnchor, constant: Layout.rankingNumberLeftOffset),
            rankingNumberLabel.centerYAnchor.constraint(equalTo: coverImageView.centerYAnchor, constant: -20),
            rankingNumberLabel.widthAnchor.constraint(equalToConstant: Layout.rankingNumberWidth),

            // Cover image view - positioned to allow number to peek out from left
    ...
    func configure(with story: CDBookInternal, ranking: Int, section: DiscoverSection? = nil) {
        self.contentMetadata = story

        // Set the ranking number (1-10)
        rankingNumberLabel.text = "\(ranking)"
        configureAccessibility(for: story, ranking: ranking, section: section)
    ...
    private func configureAccessibility(for story: CDBookInternal, ranking: Int, section: DiscoverSection?) {
        isAccessibilityElement = true
        accessibilityTraits = [.button]
        accessibilityLabel = story.title
        accessibilityValue = rankAccessibilityText(ranking: ranking, section: section)
    }

    private func rankAccessibilityText(ranking: Int, section: DiscoverSection?) -> String {
        guard let section else { return "Rank \(ranking)" }

        let sectionTitle = (section.dynamicTitle ?? section.type.localizedTitle ?? section.title).trimmingCharacters(in: .whitespacesAndNewlines)

        if let locationRange = sectionTitle.range(of: "Top 10 in ", options: [.caseInsensitive, .anchored]) {
            let locationText = sectionTitle[locationRange.upperBound...].trimmingCharacters(in: .whitespacesAndNewlines)
            if !locationText.isEmpty {
                return "Rank \(ranking) in \(locationText)"
            }
        }

        return "Rank \(ranking) in \(sectionTitle)"
    }
